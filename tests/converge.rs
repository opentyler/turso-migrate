...
async fn converge_fresh_db() {
    ...
    let normalized = normalize_for_hash(common::test_schema());
    let expected_hash = blake3::hash(normalized.as_bytes()).to_hex().to_string();
    let hash = common::get_meta(&conn, "schema_hash").await;
...
async fn crash_recovery_forces_slow_path() {
    ...
    converge(&conn, common::test_schema()).await.unwrap();
    let in_progress = common::get_meta(&conn, "migration_in_progress").await;
...
async fn policy_blocks_table_drops() {
    ...
    let result = converge_with_options(&conn, common::test_schema(), &options).await;
...
async fn dry_run_returns_plan_without_executing() {
    ...
    let in_progress = common::get_meta(&conn, "migration_in_progress").await;
    let phase = common::get_meta(&conn, "migration_phase").await;
    assert!(
        in_progress.is_none(),
...
async fn converge_with_options_returns_report() {
    ...
    let report2 = converge_with_options(&conn, common::test_schema(), &options)
        .await
        .unwrap();
...
async fn drift_detection_forces_slow_path() {
    ...
    let r1 = converge_with_options(&conn, common::test_schema(), &options)
        .await
        .unwrap();
    ...
    let r2 = converge_with_options(&conn, common::test_schema(), &options)
        .await
        .unwrap();
    assert!(
...
async fn query_only_mode_returns_read_only_error() {
    ...
    let err = converge(&conn, common::test_schema()).await.unwrap_err();
...
async fn failpoint_after_execute_preserves_crash_recovery_state() {
    ...
    let report = converge_with_options(&conn, common::test_schema(), &recover_options)
        .await
        .unwrap();
    assert_eq!(report.mode, ConvergeMode::CrashRecovery);

    let cleared = common::get_meta(&conn, "migration_in_progress").await;
